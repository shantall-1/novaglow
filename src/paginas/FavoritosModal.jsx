...
useEffect(() => {
  if (isOpen) {
    ...
    document.body.style.overflow = "auto"; // lo devuelve a la normalidad
  }

  return () => {
    document.body.style.overflow = "auto"; // por si acaso al desmontar
  };
}, [isOpen]);
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Panel lateral */}
          <motion.div
            className="fixed top-0 right-0 h-full w-[90%] sm:w-[450px] bg-white z-9999 shadow-2xl border-l border-pink-100 p-6 overflow-y-auto"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
          ...
          >
            {/* Título */}
            <div className="flex justify-between items-center mb-6 sticky top-0 bg-white pb-4 z-195">
            ...
            {favoritos.length === 0 ? (
              ...
              <div className="space-y-5 pb-8">
                {favoritos.map((item) => (
                  <motion.div
                    key={item.id}
                    className="flex gap-4 p-3 bg-white rounded-xl shadow-sm border border-pink-100 hover:shadow-md transition cursor-pointer"
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            if (item.category === "ropa")
                              return setProductoSeleccionado(item);
          ...
          <AnimatePresence>
            {productoSeleccionado && (
              ...
              >
                <motion.div
                  className="bg-white p-6 rounded-2xl shadow-xl w-[90%] max-w-md border border-pink-100"
                  initial={{ scale: 0.8 }}
                  animate={{ scale: 1 }}
                  exit={{ scale: 0.85 }}
                >
